...
# m1b: self-tune rho (rho<0)
#   - generate_windows: center = argmax(e_t in phase)
#     replaces the fixed "center = ps + rho*D" (rho=0.7) used by m0/m1
#   - peak-tile e_t + max-window score (inherited from m1)
# 算法: find_worst_window.py  (8x8 tile grid, phase-aware, RC-weighted)
# 参数: n_grid=8, k_theta=1.0, rho=-1 (self-tune), eta=0.15, k_min=2
#
# 选出 1 个 worst-case 窗口 (原始时间):
#   Window 1: 8300ns ~ 11850ns  (71 cycles, 3550ns)
#   m1b score = 1029.131   (= e_t peak @ cycle 215, 10750 ns)
# 压缩率 beta_out = 29.83% (71/238 cycles)
# 拼接 VCD 时间已重映射到 #0,本段对应 0 ~ 3550ns
# 与 m0/m1 的窗口位置 (6500~10050ns) 完全不同 → 两簇对照
#===================================================
...
# ===== RC-Tile_m1b Window 1: 原始 8300~11850ns (拼接VCD: 0~3550ns) =====
puts "========================================"
puts "=== RC-Tile_m1b Window 1: 8300ns ~ 11850ns (orig) / 0~3550ns (spliced) ==="
puts "========================================"
...
read_activity_file -format VCD ../../vcd/test_rctile_m1b.vcd \
    -scope test/u0 \
    -start 0ns \
    -end 3550ns
set_dynamic_power_simulation -resolution 1ns
report_power -o ../../db/power/avg_v20_rctile_m1b_win1
...
set_power_data -format current \
    [list ../../db/power/avg_v20_rctile_m1b_win1/dynamic_VDD.ptiavg \
          ../../db/power/avg_v20_rctile_m1b_win1/dynamic_VSS.ptiavg]
...
analyze_rail -type domain -output ../../db/rail_power_v20_rctile_m1b_win1 PD

puts "=== RC-Tile_m1b Window 1 done ==="

puts "========================================"
puts "=== All rctile_m1b windows completed ==="
puts "========================================"
exit

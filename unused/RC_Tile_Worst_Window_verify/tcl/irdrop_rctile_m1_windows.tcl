...
# Modification #1: peak-tile e_t + max-window score
#   - compute_scores: e_t[i] = max_k P_t,k[i]   (was P_total*(1+sigma_top3))
#   - score_windows: S(W) = max(e_t in W)        (was sum)
# 算法: find_worst_window.py  (8x8 tile grid, phase-aware, RC-weighted)
# 参数: n_grid=8, k_theta=1.0, rho=0.7, eta=0.15, k_min=2 (与 m0 完全一致)
#
# 选出 1 个 worst-case 窗口 (原始时间):
#   Window 1: 6500ns ~ 10050ns  (71 cycles, 3550ns)
#   m1 score = 942.992  (m0 sum-based score = 222134.471)
# 压缩率 beta_out = 29.83% (71/238 cycles)
# 拼接 VCD 时间已重映射到 #0,本段对应 0 ~ 3550ns
# 与 m0 的窗口位置完全一致 → 直接对照 IR drop 数值
#===================================================
...
# ===== RC-Tile_m1 Window 1: 原始 6500~10050ns (拼接VCD: 0~3550ns) =====
puts "========================================"
puts "=== RC-Tile_m1 Window 1: 6500ns ~ 10050ns (orig) / 0~3550ns (spliced) ==="
puts "========================================"
...
read_activity_file -format VCD ../../vcd/test_rctile_m1.vcd \
    -scope test/u0 \
    -start 0ns \
    -end 3550ns
set_dynamic_power_simulation -resolution 1ns
report_power -o ../../db/power/avg_v20_rctile_m1_win1
...
set_power_data -format current \
    [list ../../db/power/avg_v20_rctile_m1_win1/dynamic_VDD.ptiavg \
          ../../db/power/avg_v20_rctile_m1_win1/dynamic_VSS.ptiavg]
...
analyze_rail -type domain -output ../../db/rail_power_v20_rctile_m1_win1 PD

puts "=== RC-Tile_m1 Window 1 done ==="

puts "========================================"
puts "=== All rctile_m1 windows completed ==="
puts "========================================"
exit
